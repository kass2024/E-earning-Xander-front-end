...
type PayTab = "stripe" | "momo" | "promo" | "proof";
...
const LearnerPayment = () => {
  ...
  useEffect(() => {
    ...
    const loadConfig = getPaymentConfig({ courseId: courseIdNum, studentId: studentIdNum })
      .then((cfg) => {
        const mopayOk = Boolean(cfg.providers?.mopay?.configured ?? cfg.configured);
        const stripeOk = Boolean(cfg.providers?.stripe?.configured);
        setMomoConfigured(mopayOk);
        setStripeConfigured(stripeOk);
        if (!stripeOk && mopayOk) setTab("momo");
      ...
      .catch(() => {
        ...
        getStripeConfig()
          .then((cfg) => {
            setStripeConfigured(Boolean(cfg.configured));
            if (cfg.configured) setTab("stripe");
          })
  ...
  const handleStripe = async () => {
    ...
    try {
      setBusy(true);
      const res = await createPaymentCheckout(courseId, studentId);
      if (res?.url) {
      ...
      toast({
        variant: "destructive",
        title: "Stripe error",
        description: error?.response?.data?.message || "Unable to start card payment.",
      });
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-8">
      <AdminPageHeader
        ...
        title="Pay & enroll"
        description="Pay with Stripe card or Mobile Money, use a promo code, or upload payment proof."
      >
        <Button
          variant="outline"
          size="sm"
            ...
            <Card className="rounded-2xl">
              ...
              <CardContent className="space-y-4">
                <div className="flex flex-wrap gap-2">
                  {(
                    [
                      { id: "stripe" as const, label: "Card (Stripe)", icon: CreditCard },
                ...
                {tab === "stripe" && (
                  <div className="space-y-3 rounded-xl border p-4">
                    <p className="text-sm text-muted-foreground">
                      Pay the full course fee securely with Visa, Mastercard, or Amex via Stripe.
                      {!stripeConfigured ? " (Stripe is not configured yet.)" : ""}
                    </p>
                    <p className="text-sm">
                      Amount due:{" "}
                      <span className="font-semibold">
                        ${coursePrice.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD
                      </span>
                    </p>
                    <Button onClick={handleStripe} disabled={busy || !stripeConfigured}>
                ...
                {tab === "momo" && (
                  <div className="space-y-3 rounded-xl border p-4">
                    <p className="text-sm text-muted-foreground">
                      ...
                      Money is received on Xander's MoMo receiver number (Settings ? Payments).
                      {!momoConfigured ? " (Gateway not configured yet ? use Stripe or upload proof meanwhile.)" : ""}
                    </p>
                ...
                {tab === "proof" && (
                  <div className="space-y-3 rounded-xl border p-4">
                    <p className="text-sm text-muted-foreground">
                      Pay via Mobile Money using the guidelines, then upload your receipt.
