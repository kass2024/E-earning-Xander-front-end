    ...
    <div className="space-y-8">
      <AdminPageHeader eyebrow="Instructor" title="Dashboard" description="Overview of your teaching activity.">
        <Button
          ...
          disabled={refreshing}
          className="bg-[#E01C21] hover:bg-[#E79A4D] text-slate-900 font-semibold"
        >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <Card className="ring-1 ring-[#012F6B]/10 border-0 shadow-md">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-[#012F6B]">Your courses</CardTitle>
          ...
          <CardContent className="space-y-3">
            {(data?.courses ?? []).length === 0 ? (
              <p className="text-sm text-muted-foreground">
                ...
                <button
                  type="button"
                  className="text-[#012F6B] underline font-medium"
                  onClick={() => navigate("/dashboard/instructor/create-course")}
                >
                  Create one
                </button>
              ...
              (data?.courses ?? []).slice(0, 5).map((course) => (
                <div
                  key={course.id}
                  className="flex items-center justify-between gap-3 rounded-xl border border-[#012F6B]/10 bg-gradient-to-r from-white to-[#012F6B]/[0.03] p-3 dark:from-card"
                >
