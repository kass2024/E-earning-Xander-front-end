...
const AdminInstitutionManagement = () => {
  ...
  const SortIcon = ({ column }: { column: SortKey }) => {
    ...
    return sortDir === "asc" ? (
      <ArrowUp className="ml-1 inline h-3.5 w-3.5 text-[#012F6B]" />
    ) : (
      <ArrowDown className="ml-1 inline h-3.5 w-3.5 text-[#012F6B]" />
    );
  };
    ...
    <div className="space-y-6 pb-10">
      <AdminPageHeader eyebrow="Platform" title="Partner Institutions" description="Approve, manage, and preview partner dashboards.">
        ...
        <Button
          ...
          disabled={refreshing}
          className="bg-[#F2A65A] hover:bg-[#E79A4D] text-slate-900 font-semibold"
        >
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <Card
            key={item.label}
            className={cn(
              "overflow-hidden border-0 shadow-md ring-1 ring-[#012F6B]/10",
              item.accent && "ring-amber-300/60",
            )}
          >
            <div className={cn("h-1", item.accent ? "bg-amber-400" : "bg-gradient-to-r from-[#012F6B] to-[#254D81]")} />
            <CardContent className="p-4">
              <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{item.label}</p>
              <p className="mt-1 text-2xl font-bold text-[#012F6B]">{item.value}</p>
        ...
        <CardHeader className="border-b border-[#012F6B]/10 bg-gradient-to-r from-[#012F6B]/[0.05] to-transparent pb-4 space-y-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <div className="flex flex-wrap gap-2">
              <div className="flex gap-1 rounded-lg border border-[#012F6B]/15 bg-muted/40 p-1">
                {(["all", "active", "pending_approval", "disabled"] as const).map((s) => (
                  <button
                    ...
                    className={cn(
                      ...
                      statusFilter === s ? "bg-[#012F6B] text-white shadow-sm" : "text-muted-foreground hover:text-foreground",
                    )}
                  >
                ...
                {(["all", "paid", "unpaid", "promo"] as const).map((p) => (
                  <button
                    ...
                    className={cn(
                      ...
                      paymentFilter === p ? "bg-[#012F6B] text-white shadow-sm" : "text-muted-foreground hover:text-foreground",
                    )}
                  >
            ...
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <Select value={String(pageSize)} onValueChange={(v) => setPageSize(Number(v))}>
                <SelectTrigger className="h-9 w-[110px] border-[#012F6B]/15">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center px-4">
              <Building2 className="h-12 w-12 text-[#012F6B]/20 mb-3" />
            ...
            <>
              <div className="overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow className="bg-[#012F6B]/[0.04] hover:bg-[#012F6B]/[0.04]">
                      <TableHead className="w-[280px] min-w-[200px]">
                        <button type="button" className="inline-flex items-center font-semibold text-[#012F6B]" onClick={() => toggleSort("name")}>
                          Institution
                      ...
                      <TableHead>
                        <button type="button" className="inline-flex items-center font-semibold text-[#012F6B]" onClick={() => toggleSort("status")}>
                          Status
                          <SortIcon column="status" />
                        </button>
                      </TableHead>
                      <TableHead className="hidden sm:table-cell">
                        <button type="button" className="inline-flex items-center font-semibold text-[#012F6B]" onClick={() => toggleSort("payment_status")}>
                          Payment
                          <SortIcon column="payment_status" />
                        </button>
                      </TableHead>
                      <TableHead className="hidden lg:table-cell">
                        <button type="button" className="inline-flex items-center font-semibold text-[#012F6B]" onClick={() => toggleSort("total_paid_cents")}>
                          Collected
                          <SortIcon column="total_paid_cents" />
                        </button>
                      </TableHead>
                      <TableHead className="hidden xl:table-cell">
                        <button type="button" className="inline-flex items-center font-semibold text-[#012F6B]" onClick={() => toggleSort("approved_at")}>
                          Approved
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3 min-w-0">
                            {row.logo_url ? (
                              ...
                              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-[#012F6B] to-[#254D81] text-sm font-bold text-white">
              ...
              {pageCount > 1 && (
                <div className="flex flex-col items-center gap-3 border-t border-[#012F6B]/10 px-4 py-4 sm:flex-row sm:justify-between">
      ...
      <Card className="overflow-hidden border-0 shadow-md ring-1 ring-[#012F6B]/10">
        <div className="h-1 bg-gradient-to-r from-[#012F6B] via-[#254D81] to-[#F2A65A]" />
        ...
        <CardContent className="space-y-4 pb-6">
          <div className="flex flex-col gap-3 rounded-xl border border-dashed border-[#012F6B]/20 bg-[#012F6B]/[0.02] p-4 sm:flex-row sm:items-end">
            <div className="grid flex-1 gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label htmlFor="promo-code">Code</Label>
                <Input
                  id="promo-code"
                  className="h-10 font-mono uppercase border-[#012F6B]/15"
                  value={promoCode}
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  id="promo-label"
                  className="h-10 border-[#012F6B]/15"
                  value={promoLabel}
            ...
            <Button
              ...
              className="h-10 shrink-0 bg-[#012F6B] hover:bg-[#254D81] sm:w-auto w-full"
            >
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {(Array.isArray(promos) ? promos : []).map(
                (p: { id: number; code: string; uses_count: number; max_uses: number; label?: string }) => (
                  <div
                    key={p.id}
                    className="flex items-center justify-between rounded-xl border border-[#012F6B]/10 bg-white px-4 py-3 text-sm dark:bg-card"
                  >
                    <div className="min-w-0">
                      <p className="font-mono font-semibold text-[#012F6B] truncate">{p.code}</p>
