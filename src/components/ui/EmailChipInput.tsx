    ...
    <div className={cn("space-y-2", className)}>
      {(label || description) && (
        <div className="space-y-0.5">
          {label && (
            <p className="text-sm font-medium text-[#012F6B] flex items-center gap-1.5">
              <Mail className="h-3.5 w-3.5 text-[#012F6B]/70" />
              {label}
            </p>
          )}
          {description && (
      ...
      <div
        ...
        className={cn(
          ...
          "shadow-sm hover:shadow-md hover:border-[#012F6B]/25",
          focused && !error && "border-[#012F6B]/40 ring-2 ring-[#012F6B]/10 shadow-md",
        ...
        <div className="flex flex-wrap items-center gap-1.5 p-2.5 min-h-[52px]">
          {value.map((email) => (
            <span
              ...
              className={cn(
                ...
                "bg-gradient-to-r from-[#012F6B]/10 to-[#254D81]/8 text-[#012F6B]",
                "border border-[#012F6B]/15 shadow-sm",
                "animate-in fade-in zoom-in-95 duration-150",
              )}
            >
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-md bg-[#012F6B] text-[10px] font-bold text-white">
                {emailInitial(email)}
              </span>
              ...
              <button
                type="button"
                className={cn(
                  "rounded-md p-0.5 text-[#012F6B]/60 transition-colors",
                  "hover:bg-[#012F6B]/15 hover:text-[#012F6B]",
                )}
                onClick={(e) => {
                  e.stopPropagation();
          ...
          <div className="flex flex-1 min-w-[140px] items-center gap-2">
            {value.length === 0 && !draft && !focused && (
              <Mail className="h-4 w-4 shrink-0 text-[#012F6B]/25 ml-0.5" />
            )}
            <input
              ref={inputRef}
              type="email"
          ...
          <Button
            ...
            className={cn(
              ...
              "bg-[#F2A65A] text-[#012F6B] hover:bg-[#e8954a] border-0 shadow-sm",
              "disabled:opacity-40 disabled:bg-[#F2A65A]/50",
            )}
          >
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 px-0.5">
        <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
          {value.length === 0 ? (
            ...
            <span className="font-medium text-[#012F6B]/70">
        ...
        {value.length > 0 && (
          <button
            type="button"
            className="text-xs font-medium text-[#012F6B]/60 hover:text-[#012F6B] hover:underline transition-colors"
            onClick={() => onChange([])}
          >
            Clear all
          </button>
